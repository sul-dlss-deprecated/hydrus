...
Or for a quieter output on the console:
    ...
    You can look at the about page in any environment to see what Rails
    environment you are running in, other app info, as well as the app version
    number (as defined in the VERSION file).  When deployed via capistrano, a
    timestamp is added to the version number so you know when it was deployed.
...
h2. Self Service Terms Document

The Self Service Terms of Deposit Text is in THREE spots.  If you update the text, you should do it:

a. There is a word document that is not provided to users in doc/SDRSelfDepositTerms.doc.  This Word document is saved as a PDF and placed in public/SDRSelfDepositTerms.pdf, which is provided
to users when they ask to save the terms from the pop up window.  You should update both documents.

b. There is an HTML version of the terms that is shown in the popup window.  This version is in app/views/hydrus_items/_terms_of_deposit_text.html.erb
...
h2. Updating hydra-jetty submodule.
...
h2. Deployment.
    ...
    # Commit your work. Modify CHANGELOG and VERSION as needed.
    git commit -am 'blah blah'

    # Merge into develop|master, and push.
    git checkout develop && git merge working && git push origin develop
    git checkout master  && git merge develop && git push origin master
    git checkout working

    # Refresh your authenticate to Kerberos (seems to help).
    kinit && aklog

    # For a default deployment, where 'foo' is the deployment environment.
    # For a manual deployment, run the commands defined in that task.
    rake hydrus:deploy DENV=foo
