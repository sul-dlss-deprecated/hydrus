...
Or for a quieter output on the console:
	...
	You can look at the about page in any environment to see what Rails environment you are running in, other app info,
	 as well as the app version number (as defined in the VERSION file).  When deployed via capistrano, a timestamp 
	is added to the version number so you know when it was deployed.
