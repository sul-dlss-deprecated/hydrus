...
describe HydrusFormHelper, type: :helper do
  ...
  describe 'hydrus form label' do
    it 'should return the appropriate HTML when no options are sent' do
      expect(hydrus_form_label { 'My Label:' }).to have_selector 'div.col-md-1'
      expect(hydrus_form_label { 'My Label:' }).to match(/^<div.*>My Label:<\/div>$/)
    end
    it 'should apply columns passed in the options hash' do
      expect(hydrus_form_label(columns: '7') { 'My Label:' }).to have_selector 'div.col-md-7'
    end
  ...
  describe 'hydrus form value' do
    it 'should return the appropriate HTML when no options are sent' do
      expect(hydrus_form_value { "<input type='text' />" }).to have_selector 'div.col-md-8'
      expect(hydrus_form_value { "<input type='text' />".html_safe }).to have_selector "div.col-md-8 input[type='text']"
    end
    it 'should apply columns passed in the options hash' do
  ...
  describe 'hydrus form header' do
    it 'should return the appropirate HTML when no options are sent' do
      expect(hydrus_form_header { 'Title and contact' }).to(have_selector '.row .col-md-9 h4') &&
    ...
    it 'should apply the required element when the required option is sent' do
      expect(hydrus_form_header(required: true) { 'Title' }).to have_selector '.row .col-md-9 .required'
    end
  end
end
